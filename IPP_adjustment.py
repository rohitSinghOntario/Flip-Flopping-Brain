...
def EulerMaruyama(X, PAR, n_timesteps, delta, c):
    ...
    #tracking time delay - renews every delta (one event)
    t_delay = 0
    ...
    for i in range(n_timesteps):
        ...
        #updating t_delay
        if t_delay>=event_length:
            #reset delay time; new event
            t_delay = 0  
            #save n to spikes
            spikes.append(n)  
            #store spike # in event
            spike_file.write(str(n))
            spike_file.write("\n")
            #reset n 
            n=0
        else:
...
EulerMaruyama(x0, par, 20000000, 0.001, 0.1)
